...
# write('LOW.wav', 44100, 2 * crossover[:,0])
...
def stft_anal_synth(s1,s2, fs, w, N, H, 
	...
	#----------------spectral transformations------------------
		...
		gwf_c = v + (1-v) * np.exp(np.multiply((-1/(2*E_C)), np.square(np.subtract(pan_ind, si_c)) ) ) #E = 0.015
	...
	plt.ylabel('RMS Power of STFT frame')#, fontsize=16)
	label_1, = plt.plot(max_L, label='Front Left')
	label_2, = plt.plot(max_C, label='Front Center')
	label_3, = plt.plot(max_R, label='Front Right')
	label_4, = plt.plot(max_1, label='Stereo Left')
	label_5, = plt.plot(max_2, label='Stereo Right')
	plt.legend(handles=[label_1,label_2,label_3,label_4,label_5])
	#plt.legend(handles=[label_2,label_4,label_5])
	plt.show()
