...
def stft_anal_synth(s1,s2, fs, w, N, H, 
	...
	#----------------spectral transformations------------------
		...
		gwf_c = v + (1-v) * np.exp(np.multiply((-1/(2*0.001)), np.square(np.subtract(pan_ind, si_c)) ) )
		...
		new_pow = np.sum(np.square(mY1))+np.sum(np.square(mY2))+np.sum(np.square(mY_L)) +np.sum(np.square(mY_R))  # 5.1 RMS Power
#+ np.sum(np.square(mY_C))
